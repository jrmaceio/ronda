...
$convenio07 = formata_numero($dadosboleto["convenio"],7,0);
$convenio10 = formata_numero($dadosboleto["convenio"],10,0);

//agencia e conta
$agencia_codigo = $agencia ." / ". $convenio07;

if ($nossonumero == '')
{
    //nosso número (sem dv) é 8 digitos
    $nnum = $agencia . $convenio10 . formata_numero($dadosboleto["nseq"],7,0);
    //calculo do DV do nosso número
    $dv_nosso_numero = digitoVerificador_nossonumero("$nnum");
    $nossonumero_dv = formata_numero($dadosboleto["nseq"],7,0) . $dv_nosso_numero; //"$nnum$dv_nosso_numero";
    // Formata strings para impressao no boleto
    $nossonumero = substr($nossonumero_dv,0,7).'-'.substr($nossonumero_dv,7,1);
}else
{
    $nossonumero_dv = substr($nossonumero,0,7) . substr($nossonumero,8,1);
}
...
$campolivre  = "$modalidadecobranca$convenio07$nossonumero_dv$numeroparcela";
