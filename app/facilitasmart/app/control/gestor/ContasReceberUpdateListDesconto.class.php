 ...
 */
class ContasReceberUpdateListDesconto extends TPage
{
    ...
    public function __construct()
    {
        ...
        $this->addFilterField('cobranca', 'like', 'cobranca'); // filterField, operator, formField
        ...
        $column_cobranca = new TDataGridColumn('cobranca', 'Cobranca', 'left');
        ...
        $column_desconto_boleto_cobranca = new TDataGridColumn('desconto_boleto_cobranca_widget', 'Desconto Boleto Cobranca', 'left');
    ...
    public function onBeforeLoad($objects, $param)
    {
        ...
        {
            $object->desconto_boleto_cobranca_widget = new TEntry('desconto_boleto_cobranca' . '_' . $object->id);
            $object->desconto_boleto_cobranca_widget->setValue( $object->desconto_boleto_cobranca );
            $object->desconto_boleto_cobranca_widget->setSize('100%');
            $gridfields[] = $object->desconto_boleto_cobranca_widget; // important
        }
        ...
        try
        {
            ...
            foreach ($this->formgrid->getFields() as $name => $field)
            {
                if ($field instanceof TEntry)
                {
                    ...
                    {
                        $object->desconto_boleto_cobranca = $data->{$name};
                        $object->store();
                    }
                }
            }
