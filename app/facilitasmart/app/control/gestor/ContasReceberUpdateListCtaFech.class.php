 ...
 */
class ContasReceberUpdateListCtaFech extends TPage
{
    ...
    public function __construct()
    {
        ...
        $this->addFilterField('nome_responsavel', 'like', 'nome_responsavel'); // filterField, operator, formField
        $this->addFilterField('situacao', 'like', 'situacao'); // filterField, operator, formField
        $this->addFilterField('conta_fechamento_id', '=', 'conta_fechamento_id'); // filterField, operator, formField
        ...
        $this->form->addFields( [ new TLabel('Nome Responsavel') ], [ $nome_responsavel ] );
        $this->form->addFields( [ new TLabel('Situacao') ], [ $situacao ] );
        $this->form->addFields( [ new TLabel('Conta Fechamento Id') ], [ $conta_fechamento_id ] );
        ...
        $column_conta_fechamento_id = new TDataGridColumn('conta_fechamento_id_widget', 'Conta Fechamento Id', 'right');
    ...
    public function onBeforeLoad($objects, $param)
    {
        ...
        {
            $object->conta_fechamento_id_widget = new TEntry('conta_fechamento_id' . '_' . $object->id);
            $object->conta_fechamento_id_widget->setValue( $object->conta_fechamento_id );
            $object->conta_fechamento_id_widget->setSize('100%');
            $gridfields[] = $object->conta_fechamento_id_widget; // important
        }
        ...
        try
        {
            ...
            foreach ($this->formgrid->getFields() as $name => $field)
            {
                if ($field instanceof TEntry)
                {
                    ...
                    {
                        $object->conta_fechamento_id = $data->{$name};
                        $object->store();
                    }
                }
            }
