 ...
 */
class GraficoInadimplenciaBlocoQuadraDash extends TPage
{
    /**
     * Class constructor
     * Creates the page
    ...
    function __construct( $show_breadcrumb = true )
    {
        ...
        $colunas1 = $conn1->query("SELECT 
                                    ...
                                    extract(month from contas_receber.dt_vencimento) AS mes, 
                                    extract(year from contas_receber.dt_vencimento) AS ano, 
                                    unidade.bloco_quadra 
                                   from contas_receber 
                                   ...
                                   group by bloco_quadra, mes, ano
                                    order by bloco_quadra
                                ");
        
        // pega o último fechamento     
        $dados = array();
        $dados[] = [ 'Bloco-Quadra', 'R$' ];

        $total_inadimplencia = 0;
        
        foreach($colunas1 as $coluna1)
        {
            $dados[] = [ $coluna1['bloco_quadra'], (float)$coluna1['valor'] ];
        ...
        $panel = new TPanelGroup('Gráfico Inadimplência Bloco - Quadra');
