 ...
 */
class ListagerarboletosReport extends TPage
{
    protected $form; // form
    ...
    function onGenerate()
    {
        try
        {
            ...
            if ($objects)
            {
                $widths = array(30,50,300,50,50,50,250);
                ...
                $tr->addStyle('title', 'Arial', '9', 'B',   '#ffffff', '#A3A3A3');
                $tr->addStyle('datap', 'Arial', '8', '',    '#000000', '#EEEEEE');
                $tr->addStyle('datai', 'Arial', '8', '',    '#000000', '#ffffff');
                $tr->addStyle('header', 'Arial', '15', '',   '#ffffff', '#6B6B6B');
                ...
                // add a header row
                $tr->addRow();
                $tr->addCell($condominio->resumo,'center', 'header', 7);
                $tr->addRow();
                $tr->addCell('Listagem para Geração de Boletos', 'center', 'title', 7);
                ...
                $tr->addRow();
                $tr->addCell('Total de Unidades : '.$total_unidades, 'center', 'footer', 7);
                ...
                // stores the file
                if (!file_exists("app/output/Listagerarboletos.{$format}") OR is_writable("app/output/Listagerarboletos.{$format}"))
                {
                    $tr->save("app/output/Listagerarboletos.{$format}");
                }
                else
                {
                    throw new Exception(_t('Permission denied') . ': ' . "app/output/Listagerarboletos.{$format}");
                }
                
                // open the report file
                parent::openFile("app/output/Listagerarboletos.{$format}");
