...
class ContasReceberListagem extends TPage
{
    ...
    public function __construct($param)
    {
        ...
        //                TSession::getValue('id_condominio')  . ' - ' . TSession::getValue('resumo')));
        ...
        $checktodos->title = 'Select/UnSelect all';
        $checktodos->onclick = "$('input:checkbox').not(this).prop('checked',this.checked);";              

        
        // creates the datagrid columns
        $column_check = new TDataGridColumn('check',$checktodos,'center', 40);
        ...
        $action3 = new TDataGridAction(['ContasReceberListagemAux', 'onInputDialog']);
        $action3->setLabel('Boleto');
        $action3->setField('id');
        $action3->setImage('far:arrow-alt-circle-down orange');
        ...
        $action_group->addHeader('Movimento');
        ...
        // -- gera boleto coletivo -- varios -- // // -- novos
        $this->boletoButton = new TButton('boleto_collection');
        $this->boletoButton->setAction(new TAction(array($this, 'onBoletoCollection')), 'Boleto Selecioando');
        $this->boletoButton->setImage('fas:check blue');
        $this->formgrid->addField($this->boletoButton);

        // creates the delete collection button
        $this->deleteButton = new TButton('delete_collection');
        $this->deleteButton->setAction(new TAction(array($this, 'onDeleteCollection')), 'Deleta Selecionado');
        $this->deleteButton->setImage('fas:times red');
        $this->formgrid->addField($this->deleteButton); //delete boleto geral

        // -- novos
        $gridpack = new TPanelGroup;
        $gridpack->style = 'width: 100%';
        $gridpack->add($this->formgrid);
        $gridpack->addFooter(THBox::pack( $this->boletoButton )); 
    ...
    static function onBoletoCollection( $param )
    {
        $data = $param;
        if ($data) 
        {
            ...
            $parameters = array();
            $parameters[ 'static' ] = '1';
            $parameters[ 'id' ] = $checks[0];
            $parameters[ 'checks' ] = $checks;
            AdiantiCoreApplication::loadPage('ContasReceberListagemAux','onInputDialog',(array) $parameters);
        }
    }
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            if ($objects)
            {
                ...
                {
                    $item = new StdClass;
                    $object->check = new TCheckButton('check[]');
                    $object->check->setIndexValue($object->id);
                    $item->check = $object->check;
                    ...
                    */
                    ...
                    //-- novos
                    //$this->formgrid->addField($item->check);
                }
            }
