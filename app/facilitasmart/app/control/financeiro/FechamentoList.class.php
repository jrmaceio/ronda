...
class FechamentoList extends TPage
{
    ...
    public function __construct()
    {
        ...
        //$this->form->addAction(_t('Edit'),  new TAction(array('FechamentoForm', 'onEdit')), 'fa:plus red');
        ...
        // create Abrir action
        $action_abrir = new TDataGridAction(array($this, 'AbrirCollection'));
        $action_abrir->setButtonClass('btn btn-default');
        $action_abrir->setLabel('Abrir');
        $action_abrir->setImage('fa:folder-open green');
        $action_abrir->setField('id');
        $this->datagrid->addAction($action_abrir);
        
        // create Abrir action
        $action_fechar = new TDataGridAction(array($this, 'FecharCollection'));
        $action_fechar->setButtonClass('btn btn-default');
        $action_fechar->setLabel('Frchar');
        $action_fechar->setImage('fa:window-close red');
        $action_fechar->setField('id');
        $this->datagrid->addAction($action_fechar);
    ...
    public function fecharCollection($param)
    {
        try
        {
            ...
            TTransaction::close(); // close the transaction
            $this->onReload( $param ); // reload the listing
            new TMessage('info', 'Movimento fechado para edições'); // success message
        }
        catch (Exception $e) // in case of exception
        {
            new TMessage('error', $e->getMessage()); // shows the exception error message
    ...
    public function abrirCollection($param)
    {
        try
        {
            ...
            TTransaction::close(); // close the transaction
            $this->onReload( $param ); // reload the listing
            new TMessage('info', 'Movimento aberto para edições'); // success message
        }
        catch (Exception $e) // in case of exception
        {
            new TMessage('error', $e->getMessage()); // shows the exception error message
